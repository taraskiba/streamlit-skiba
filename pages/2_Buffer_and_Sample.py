...
st.header(
    "Optional Preliminary Step to the Point or Aggregated Point Extraction Modules."
)
...
with col2:
    sample_size = st.number_input(
        "Step 4: Number of samples to pull",
        min_value=0,
        value=5,
        step=1,
        key="sample_size)",
        ...
        if uploaded_file is not None:
            ...
            if not geedata:
                ...
                if csv:
                    ...
                    st.download_button(
                        label="Download Results", data=csv, file_name=file_name
                    )
                else:
                    st.error(
